/**
		    ...
		    Star.prototype.draw = function() {
		    	...
		    	//starCtx.shadowBlur = this.r * 2;
		    	starCtx.beginPath();
		    	starCtx.arc(this.x, this.y, this.r, 0, 2 * Math.PI, false);
		    	starCtx.closePath();
		    	starCtx.fill();
		    }

		    Star.prototype.move = function() {
		    	this.y -= .15 + starParams.backgroundSpeed/100;
		    	if (this.y <= -10) this.y = starHeight + 10;
		    	this.draw();
		    }
		    ...
		    Dot.prototype.link = function() {
		    	...
		    	starCtx.strokeStyle = this.linkColor;
		    	starCtx.moveTo(previousDot1.x, previousDot1.y);
		    	starCtx.beginPath();
		    	starCtx.lineTo(this.x, this.y);
		    	if (previousDot2 != false) starCtx.lineTo(previousDot2.x, previousDot2.y);
		    	if (previousDot3 != false) starCtx.lineTo(previousDot3.x, previousDot3.y);
		    	starCtx.stroke();
		    	starCtx.closePath();
		    }
		    ...
		    Dot.prototype.move = function() {
		    	...
		    	this.color = "rgba(255,255,255,"+this.a+")";
		    	this.linkColor = "rgba(255,255,255,"+this.a/4+")";
		    	this.x = this.x + Math.cos(degToRad(this.dir))*(this.speed+starParams.dotsSpeed/100),
		    	this.y = this.y + Math.sin(degToRad(this.dir))*(this.speed+starParams.dotsSpeed/100);
		    ...
		    var starCanvas  = document.getElementById('star-fall');
		    var starCtx = starCanvas.getContext('2d');
		    var starWidth;
		    var starHeight;
		    var starMouseMoving = false;
		    var starMouseMoveChecker;
		    var starMouseX;
		    var starMouseY;
		    var stars = [];
		    ...
		    var gui;
		    gui = new dat.GUI();
		    gui.add(starParams, 'maxDistFromCursor').min(0).max(100).step(10).name('Size');
		    gui.add(starParams, 'dotsSpeed').min(0).max(100).step(.5).name('Speed');
		    gui.add(starParams, 'backgroundSpeed').min(0).max(150).step(1).name('Background speed');
		    gui.open();
		    ...
		    function setCanvasSize() {
		    	starWidth = document.getElementById('header').offsetWidth,
		        starHeight = document.getElementById('header').offsetHeight;                      

		    	starCanvas.setAttribute("width", starWidth);
		    	starCanvas.setAttribute("height", starHeight);
		    }
		    ...
		    function init() {
		    	...
		    	for (var i = 0; i < initStarsPopulation; i++) {
		    		stars[i] = new Star(i, Math.floor(Math.random()*starWidth), Math.floor(Math.random()*starHeight));
		    		//stars[i].draw();
		    	}
		    ...
		    starCanvas.onmousemove = function(e){
		    	starMouseMoving = true;
		    	starMouseX = e.clientX;
		    	starMouseY = e.clientY;
		    	clearInterval(starMouseMoveChecker);
		    	starMouseMoveChecker = setTimeout(function() {
		    		starMouseMoving = false;
		    	}, 100);
		    }


		    function drawIfMouseMoving(){
		    	if (!starMouseMoving) return;

		    	if (dots.length == 0) {
		    		dots[0] = new Dot(0, starMouseX, starMouseY);
		    		dots[0].draw();
		    		return;
		    	}
		    	...
		    	var diffX = Math.abs(prevX - starMouseX);
		    	var diffY = Math.abs(prevY - starMouseY);

		    	if (diffX < dotsMinDist || diffY < dotsMinDist) return;

		    	var xVariation = Math.random() > .5 ? -1 : 1;
		    	xVariation = xVariation*Math.floor(Math.random()*starParams.maxDistFromCursor)+1;
		    	var yVariation = Math.random() > .5 ? -1 : 1;
		    	yVariation = yVariation*Math.floor(Math.random()*starParams.maxDistFromCursor)+1;
		    	dots[dots.length] = new Dot(dots.length, starMouseX+xVariation, starMouseY+yVariation);
